...
directory <- "C:/Users/user/Desktop/marion/ACE2/"
dir(directory)
...
condition = c("african", "non-african")
...
# perform an exact test for the difference in expression between the two conditions “treated” and “control”
...
plotSmear(dgeTestFilt,
          de.tags = rownames(resFilt$table)[which(resFilt$table$FDR<0.10)])
...
resFilt$table$diffexpressed[resFilt$table$logFC > 1 & resFilt$table$FDR < 0.1] <- "Up"

# if log2Foldchange < -0.6 and pvalue < 0.05, set as "DOWN"
resFilt$table$diffexpressed[resFilt$table$logFC < -1 & resFilt$table$FDR < 0.1] <- "Down"
...
volc = ggplot(input, aes(logFC, -log10(PValue))) + #volcanoplot with logFC versus pvalue
  geom_point() +
  theme_classic()+
  geom_vline(xintercept=c(-1.5, 1.8), col="cyan", linetype="dashed") +
  geom_hline(yintercept=-log10(0.002), col="cyan" , linetype="dashed")+
...
volc+geom_text_repel(data=head(input, 40), aes(label=gene), size = 2) #adding text for the top 20 genes
